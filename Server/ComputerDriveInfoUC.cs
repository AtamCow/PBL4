...
namespace Server
{
    public partial class ComputerDriveInfoUC : UserControl
    {
        private int hotTabIndex = -1;
        private double scalingFactor = 1;
        private DiskSystemInformation? DiskInfo = null;

        public ComputerDriveInfoUC()
        {
            GetHDPISettings();
            InitializeComponent();
            InitDiskTabListUI();
        ...
        public void SetDiskInfo(DiskSystemInformation DiskInfo)
        {
        ...
        public void Load()
        {
            ...
            foreach (var disk in DiskInfo.PhysicalDisk)
            {
                ...
                if (ContainsCapacity(disk.Name))
                {
                    myDiskControl.SetDiskName($"{disk.Name}");

                }
                else
                {
                    myDiskControl.SetDiskName($"{disk.Name} {FormatBytes1000(disk.Size)}");
                }
        ...
        private void tabControl1_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            PhysicalDiskInformation disk = DiskInfo.PhysicalDisk[e.Index];
